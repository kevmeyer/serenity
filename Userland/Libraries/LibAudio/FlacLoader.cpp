...
#define FLAC_VERIFY(check, category, msg)                                                                                                             \
    do {                                                                                                                                              \
        if (!(check)) {                                                                                                                               \
            return LoaderError { category, TRY(m_stream->tell()), MUST(FlyString::from_utf8(ByteString::formatted("FLAC header: {}", msg).view())) }; \
        }                                                                                                                                             \
    } while (0)
    ...
    if (!maybe_target_seekpoint.has_value()) {
        ...
        if (sample_index - target_seekpoint.sample_index > sample_index - m_loaded_samples) {
            ...
            if (m_stream->seek(static_cast<i64>(position), SeekMode::SetPosition).is_error())
                return LoaderError { LoaderError::Category::IO, m_loaded_samples, MUST(FlyString::from_utf8(ByteString::formatted("Invalid seek position {}", position).view())) };
...
#define FLAC_VERIFY(check, category, msg)                                                                                                                                      \
    do {                                                                                                                                                                       \
        if (!(check)) {                                                                                                                                                        \
            return LoaderError { category, static_cast<size_t>(m_current_sample_or_frame), MUST(FlyString::from_utf8(ByteString::formatted("FLAC header: {}", msg).view())) }; \
        }                                                                                                                                                                      \
    } while (0)
...
ErrorOr<u8, LoaderError> FlacLoaderPlugin::convert_bit_depth_code(u8 bit_depth_code)
{
    ...
    case 7:
        return 32;
    default:
        return LoaderError { LoaderError::Category::Format, static_cast<size_t>(m_current_sample_or_frame), MUST(FlyString::from_utf8(ByteString::formatted("Unsupported sample size {}", bit_depth_code).view())) };
    }
}
...
ErrorOr<Vector<i64>, LoaderError> FlacLoaderPlugin::decode_fixed_lpc(FlacSubframeHeader& subframe, BigEndianInputBitStream& bit_input)
{
    ...
    case 4:
        ...
        break;
    default:
        return LoaderError { LoaderError::Category::Format, static_cast<size_t>(m_current_sample_or_frame), MUST(FlyString::from_utf8(ByteString::formatted("Unrecognized predictor order {}", subframe.order).view())) };
    }
    return decoded;
}
